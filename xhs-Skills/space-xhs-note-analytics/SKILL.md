...
- 封面改稿 → `space-xhs-cover-html`；标题改写 → `space-xhs-title`
- 抓取平台数据、刷量刷互动、批量起号
...
| 表现 | 归因 | 该改什么 |
|---|---|---|
| CTR 低 + 封面文字多/小 | 信息流里缩略图看不清 | 封面主文案压到 6–10 字，字号加大 → `space-xhs-cover-html` |
...
如果动作指向封面/标题，直接接力：「具体怎么改封面，可以用 `space-xhs-cover-html` 出几版方案」。
