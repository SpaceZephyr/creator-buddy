...
| 页码 | 页面任务 | 主标题 | 结构 | 读者拿走什么 |
|---|---|---|---|---|
| 01 | 点击 | 财务人怎么用 Codex | 封面 | 明确主题 |
| 02 | 留人 | 先看完整工作流 | 总览 | 知道后续路径 |
| 03 | 准备 | 准备 3 份输入 | 清单 | 可照着准备 |
...
```
...
生图模型能生成文字，但信息越密，错字和错行概率越高。每页遵守：

- 主标题：8–18 个汉字，最多 2 行。
- 副标题：0–26 个汉字。
- 正文：3–6 个模块，每个模块 4–18 个汉字。
- 全页建议不超过 90 个汉字；流程页或对比页最多 120 个汉字。
- 只保留会被读者真正阅读的文字。
- 所有文字逐字写入提示词的 `Text (verbatim)`，不可让模型自行改写。

正文过多时先拆成更多页，不缩成密密麻麻的小字。
...
- 黑色粗标题是第一视觉，紫蓝和青柠绿只强调关键词。
- 中部用大圆角白卡承载信息；卡片内使用短句、编号和线性图标。
